...
export default class EventService {
    getEvents(): Observable<ConferenceEvent[]> {
        const subject = new Subject<ConferenceEvent[]>();
    ...
    updateEvent( event: ConferenceEvent ) {
        const index = events.findIndex( x => x.id === event.id );

        events[ index ] = event;
    }
    searchSessions( searchTerm: string ): Observable<EventSession[]> {
        const term = searchTerm.toLocaleLowerCase();
        let results: EventSession[] = [];

        events.forEach( event => {
            if ( event.sessions.length === 0 ) { return; }
            let matchingSessions: any = event.sessions.filter( session => session.name.toLocaleLowerCase().indexOf( term ) > -1 );
        ...
        const emitter = new EventEmitter<EventSession[]>( true );
